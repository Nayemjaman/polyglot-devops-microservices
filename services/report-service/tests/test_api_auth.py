...
def test_all_report_api_routes_require_authenticated_user_dependency() -> None:
    app = create_app()
...
@pytest.mark.asyncio
async def test_report_api_without_authorization_is_rejected() -> None:
    app = create_app()

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.get("/api/reports/monthly?year=2026&month=5")
