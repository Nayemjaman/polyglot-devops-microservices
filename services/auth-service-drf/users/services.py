...
async def authenticate_user(email, password):
    try:
        user = await User.objects.select_related('profile').aget(email__iexact=email, is_active=True)
...
async def get_authenticated_user(request):
    ...
    try:
        return await User.objects.select_related('profile').aget(id=user_id, is_active=True)
    except User.DoesNotExist as exc:
